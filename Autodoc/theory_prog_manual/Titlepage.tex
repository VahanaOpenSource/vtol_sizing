...
The main advantages of \hydra \spc over other sizing codes are \textbf{flexibility}, \textbf{speed} and reliance on only \textbf{open-source tools}. With the majority of the code written in an interpreted language, i.e., \python, modules can be prototyped and added quickly. Subsequently select parts of the code can be ported to \ty{Fortran} or \ty{C} (i.e., compiled languages) and wrapped for execution sppeed. Using a combination of \ty{OpenMP}, \ty{MPI} and algorithmic acceleration, up to 2000 designs can be generated per second on a standard desktop computer with 8 cores. 
